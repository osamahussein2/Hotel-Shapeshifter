...
public partial class TelemetryLogger : MonoBehaviour
{
    ...
    {
        #if UNITY_EDITOR
        // Handles resetting the telemetry service when entering play mode
        // even when domain reload is turned off in project settings.
        [RuntimeInitializeOnLoadMethod(RuntimeInitializeLoadType.BeforeSplashScreen)]
        static void ResetStaticContent() {
            TelemetryLogger._loggers = new();
            _serviceMap = new();
        }
        #endif
        ...
        static Dictionary<TelemetrySettings, Service> _serviceMap = new();
        ...
        public static Service Connect(TelemetryLogger logger, System.Action<int> onSuccess, System.Action<string> onFail)
        {
            ...
            if (!_serviceMap.TryGetValue(logger.Settings, out var service))
            {
                ...
                service.OnConnectionSuccess += onSuccess;
                service.OnConnectionFail += onFail;
        ...
        void Initialize(TelemetrySettings settings)
        {
            ...
            {
                NetworkingMode.BuildAndPlayModeInEditor => true,
                NetworkingMode.BuildOnly => Application.isEditor == false,
                NetworkingMode.Disabled => false,
                _ => false
            };
            if (_settings.debugLogging <= LoggingPolicy.Connection)
                Debug.Log($"Loaded {settings.name}: {(_enableNetwork ? "logging to server enabled" : "logging to local debug console only" )}.");
        }
        ...
        static bool IsDataMissing<T>(TelemetryEvent<T> source, string json) {
            return json.IndexOf("\"data\":") < 0;
        }

        static readonly System.Text.RegularExpressions.Regex DATA_PURGE = new("\"data\":.*}\\Z", System.Text.RegularExpressions.RegexOptions.Compiled);
        ...
        public void TryLog<T>(TelemetryLogger logger, TelemetryEvent<T> telemetry)
        {
            ...
            if (telemetry.data == null) {
                json = DATA_PURGE.Replace(json, "\"data\": null}");
            }            
            #if UNITY_EDITOR
            else if (_settings.debugLogging <= LoggingPolicy.ErrorsOnly) {
                if (telemetry.NeedsSerializationCheck() && IsDataMissing(telemetry, json)) {
                    Debug.LogError($"Trying to log event '{telemetry.eventType}' with non-serializable data payload type {typeof(T)}. Be sure to use the [System.Serializable] attribute on any custom class/struct you want to use as telemetry data.");
                }
            }
            #endif

            _queuedRequests.Enqueue(json);
        ...
        IEnumerator WakeAndAuthenticate(string section)
        {
            if (!_enableNetwork) {
                ...
                var login = new AuthenticationRequest
                {
                    ...
                    section = section,
                    timecode = System.DateTimeOffset.UtcNow.ToUnixTimeMilliseconds(),
                    setupId = _settings.SetupId
                };
