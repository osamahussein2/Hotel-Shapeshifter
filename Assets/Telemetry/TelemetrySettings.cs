...
{   
    ...
    [Header("Telemetry Version 4.2")]

    [Tooltip("This is used as the name of the database table to log events into.")]
    public string userName = "Your SLATE user name or team name";
    ...
    public string secret = "Your student # or team passphrase";
    ...
    [SerializeField] string _version = "test";
    /// <summary>
    ...
    [SerializeField] string _serverURL = "https://telemetry-dg-13578a3082a2.herokuapp.com/";
    ...
    public NetworkingMode serverCommunication = NetworkingMode.BuildOnly;
    ...
    void OnValidate() {
        if (string.IsNullOrWhiteSpace(SetupId)) {
            string folderName = Application.dataPath.Split('/')[^2];
            SetupId = $"{System.Environment.MachineName} > {folderName} / {Application.productName} @ {System.Environment.UserName}-{Random.Range(0, int.MaxValue)}"; 
        }
    }
#endif
